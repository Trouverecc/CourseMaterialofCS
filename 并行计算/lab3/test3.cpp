...
int main(int argc, char *argv[]) {
    ...
    if (rank != 0) {
        MPI_Send(&local_result[0], (end_row - start_row) * N, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    } 
    else {
        ...
        for (int source = 1; source < size; ++source) {
            ...
            MPI_Recv(&recv_buffer[0], (end_row1 - start_row1) * N, MPI_DOUBLE, source, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
